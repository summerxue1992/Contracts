...
contract HASH is Ownable {
    ...
    struct WinningResult {
        ...
        bool    hasReward;              // 是否结算
        uint[]  numberList;            // 三个数字数组
        uint    sumResult;             //三个数字相加
        uint    bankerNumber;            // 庄的数字
        uint    idleNumber;              // 闲的数字
    ...
    mapping (uint => WinningResult) public WinningMap;          // 往期记录
